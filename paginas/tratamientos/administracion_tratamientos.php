...
<head>
    ...
    <link rel="stylesheet" href="administracion_tratamientos.css">
...
<body>
   <header>
    ...
    <div class="treatment-table">
        <h2>Administrar Tratamientos</h2><br>
        <table>
            <thead>
                <tr>
                    <th>ID</th>
            ...
            <tbody>
                ...
                $query = "SELECT t.id, p.nombre AS paciente, d.nombre AS doctor, t.tipo_tratamiento, t.fecha_inicio
                          FROM tratamientos t
                          JOIN pacientes p ON t.paciente_id = p.id
                          JOIN doctores d ON t.doctor_id = d.id_doc";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>
                                <a href='ver_tratamiento.php?id={$row['id']}' class='action-view' title='Ver'><i class='fas fa-eye'></i></a>
                                <a href='actualizar_tratamiento.php?id={$row['id']}' class='action-edit' title='Modificar'><i class='fas fa-edit'></i></a>
                                <a href='#' class='action-delete' title='Eliminar' onclick='confirmDelete({$row['id']})'><i class='fas fa-trash'></i></a>
                              </td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='6'>No hay tratamientos en la base de datos.</td></tr>";
                }
