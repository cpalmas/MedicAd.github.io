...
<head>
    <meta charset="UTF-8">
    <title>Administración de Citas Ginecológicas</title>
    <link rel="stylesheet" href="administracion_ginecologia.css">
...
<!-- -----------------------------------------------------------------------------------------tabla administracion-->
    ...
    <div class="ginecology-table">
        <h2>Registros Ginecologicos</h2>
        <table>
            <thead>
                <tr>
                    <th>ID</th>
            ...
            <tbody>
                ...
                $query = "SELECT g.id, p.nombre AS paciente, d.nombre AS doctor, g.fecha_cita_ginecologica, g.motivo_consulta
                          FROM ginecologia g
                          JOIN pacientes p ON g.paciente_id = p.id
                          JOIN doctores d ON g.doctor_id = d.id_doc";
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>";
                        ...
                        echo "<td>{$row['fecha_cita_ginecologica']}</td>";
                        echo "<td>{$row['motivo_consulta']}</td>";
                        echo "<td>
                                <a href='ver_doctor.php?id={$row['id']}' class='action-view' title='Ver'><i class='fas fa-eye'></i></a>
                                <a href='actualizar_ginecologia.php?id={$row['id']}' class='action-edit' title='Modificar'><i class='fas fa-edit'></i></a>
                                <a href='#' class='action-delete' title='Eliminar' onclick='confirmDelete({$row['id']})'><i class='fas fa-trash'></i></a>
                              </td>";
                        echo "</tr>";
                }
            } else {
                    echo "<tr><td colspan='5'>No hay registros ginecologicos en la base de datos.</td></tr>";
            }
