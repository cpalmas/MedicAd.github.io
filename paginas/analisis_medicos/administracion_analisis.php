...
<head>
    <meta charset="UTF-8">
    <title>Administración de Análisis Médicos</title>
    <link rel="stylesheet" href="administracion_analisis.css">
...
<body>
    ...
    <div class="analysis-table">
        <h2>Analisis Medicos Registrados</h2>
        <table>
            <thead>
                <tr>
                    <th>ID</th>
          ...
          JOIN doctores d ON a.doctor_id = d.id_doc";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>{$row['tipo_analisis']}</td>";
        echo "<td>{$row['fecha_analisis']}</td>";
       
                        echo "<td>
                                <a href='ver_analisis.php?id={$row['id']}' class='action-view' title='Ver'><i class='fas fa-eye'></i></a>
                                <a href='actualizar_analisis.php?id={$row['id']}' class='action-edit' title='Modificar'><i class='fas fa-edit'></i></a>
                                <a href='#' class='action-delete' title='Eliminar' onclick='confirmDelete({$row['id']})'><i class='fas fa-trash'></i></a>
                              </td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='5'>No hay registros de analisis en la base de datos.</td></tr>";
                }
