...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $sql = "DELETE FROM odontologia WHERE id = ?";
    $stmt = $db->prepare($sql);
    $stmt->bind_param('i', $id);

    if ($stmt->execute()) {
        // Redirigir a la página de la tabla con un mensaje de éxito
        header("Location: administracion_odontologia.php?msg=success");
    } else {
        // Redirigir a la página de la tabla con un mensaje de error
        header("Location: administracion_odontologia.php?msg=error");
    }
    ...
    header("Location: administracion_odontologia.php?msg=invalid");
}
?>
